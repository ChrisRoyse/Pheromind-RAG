...
// Removed unused import: use rustc_hash::FxHashMap;
...
impl Default for FusionConfig {
    fn default() -> Self {
        Self {
            ...
            semantic_score_factor: 1.2, // Increased to favor semantic matches more
            min_semantic_similarity: 0.4, // Higher threshold for better quality
        }
    }
}
...
impl SimpleFusion {
        ...
        for (_idx, semantic) in semantic_matches.into_iter().enumerate() {
            ...
            if !file_has_exact {
                ...
                if seen.insert(key) {
                    ...
                    // Skip semantic matches below quality threshold
                    ...
                    results.push(FusedResult {
                        ...
                        score: self.calculate_semantic_score(similarity), // Improved semantic scoring
            ...
            if !file_has_better_match {
                ...
                if seen.insert(key) {
                    ...
                    results.push(FusedResult {
                        ...
                        line_number: None,
                        chunk_index: Some(semantic.chunk_index as usize),
                        score: self.calculate_semantic_score(similarity) * 0.9, // Slightly lower for multi-way fusion
                        match_type: MatchType::Semantic,
                        content: semantic.content,
                        start_line: semantic.start_line as usize,
                        end_line: semantic.end_line as usize,
    ...
    pub fn fuse_all_results_with_bm25(
            ...
            if !file_has_better_match {
                ...
                if seen.insert(key) {
                    ...
                    results.push(FusedResult {
                        ...
                        line_number: None,
                        chunk_index: Some(semantic.chunk_index as usize),
                        score: self.calculate_semantic_score(similarity) * 0.85, // Configurable semantic reduction for 4-way fusion
                        match_type: MatchType::Semantic,
                        content: semantic.content,
                        start_line: semantic.start_line as usize,
                        end_line: semantic.end_line as usize,
        ...
        // Apply weighted fusion scoring with configured weights
        // These weights will be passed from UnifiedSearcher configuration
    ...
    /// Apply weighted fusion with external configuration
    pub fn apply_configured_fusion(
        ...
        self.apply_weighted_fusion(results, exact_weight, bm25_weight, semantic_weight, symbol_weight);
    }
    ...
    /// Calculate improved semantic score with non-linear scaling
    /// This gives better discrimination between good and excellent matches
    fn calculate_semantic_score(&self, similarity: f32) -> f32 {
        // Apply non-linear scaling to better discriminate semantic quality
        // High similarities get boosted more than linear scaling would provide
        let base_score = similarity * self.config.semantic_score_factor;
        
        // Apply sigmoid-like curve to amplify high similarities
        if similarity > 0.7 {
