...
impl UnifiedSearcher {
    ...
    pub async fn new_with_backend_and_config(project_path: PathBuf, db_path: PathBuf, backend: SearchBackend, include_test_files: bool) -> Result<Self> {
        ...
        // Get search configuration for parameter-driven initialization
        let search_config = config.search.clone();
        
        // Initialize BM25 components with configured parameters
        let mut bm25_engine = BM25Engine::with_params(search_config.bm25_k1, search_config.bm25_b);
        let bm25_index_path = db_path.join(&config.bm25_index_path);
        let bm25_state_path = bm25_index_path.join("bm25_state.bin");
        
        // Load existing BM25 state - corruption or IO failures are fatal
        bm25_engine.load_from_disk(&bm25_state_path).await
            .map_err(|e| anyhow::anyhow!("Failed to load BM25 engine state: {}. State corruption is not acceptable.", e))?;
        
        let bm25_engine = Arc::new(RwLock::new(bm25_engine));
        ...
        Ok(Self {
            ...
            cache: SearchCache::new(search_config.search_cache_size),
    ...
    fn get_cached_lowercase(&self, term: &str) -> String {
        ...
        let lowercased = term.to_lowercase();
        {
            ...
            // Prevent unbounded growth - use configured term cache size
            if cache.len() < self.search_config.term_cache_size {
    ...
    pub async fn search(&self, query: &str) -> Result<Vec<SearchResult>> {
        ...
        // Apply configured fusion weights before ranking optimization
        self.fusion.apply_configured_fusion(
            &mut fused, 
            self.search_config.fusion_exact_weight,
            self.search_config.fusion_bm25_weight,
            self.search_config.fusion_semantic_weight,
            self.search_config.fusion_symbol_weight,
        );
    ...
    async fn search_semantic(&self, query: &str) -> Result<Vec<crate::storage::lancedb_storage::LanceEmbeddingRecord>> {
        ...
        // Search in vector database with configured limit
        let storage = self.storage.read().await;
        storage.search_similar(query_embedding, self.search_config.semantic_max_results).await
            .map_err(|_e| SearchError::IndexNotReady {
                index_name: "Semantic".to_string(),
            }.into())
    }
    ...
    async fn search_bm25(&self, query: &str) -> Result<Vec<BM25Match>> {
        ...
        if query.trim().is_empty() {
            return Err(SearchError::QueryInvalid {
                message: "Query cannot be empty for BM25 search".to_string(),
                query: query.to_string(),
            }.into());
        }
        
        // CRITICAL FIX: Process query using same tokenizer as indexing
        // This ensures query terms match indexed terms exactly
        let processed_tokens = self.text_processor.tokenize_code(query, None);
        let query_terms: Vec<String> = processed_tokens
            .into_iter()
            .map(|token| token.text.to_lowercase()) // Ensure consistent lowercasing
            .collect();
        
        if query_terms.is_empty() {
            return Err(SearchError::QueryInvalid {
                message: "Query contains no searchable terms after processing".to_string(),
                query: query.to_string(),
            }.into());
        }
        
        let engine = self.bm25_engine.read().await;
        engine.search_with_terms(&query_terms, self.search_config.bm25_max_results)
            .map_err(|e| SearchError::IndexNotReady {
    ...
    pub async fn index_file(&self, file_path: &Path) -> Result<()> {
        ...
        if self.bm25_enabled {
            ...
            // Save BM25 engine state for persistence
            let engine = self.bm25_engine.read().await;
            engine.save_to_disk(&self.bm25_state_path).await?;
