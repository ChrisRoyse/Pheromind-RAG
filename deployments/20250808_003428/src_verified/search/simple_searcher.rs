...
impl SimpleSearcher {
    ...
    pub async fn search(&self, query: &str) -> Result<Vec<SimpleSearchResult>> {
        ...
        if let Some(ref engine) = self.bm25_engine {
            match engine.search(query, self.config.bm25_max_results) {
                Ok(results) => {
                    engines_used.insert("bm25".to_string());
                    for result in results {
                        all_results.push(SimpleSearchResult {
                            path: result.doc_id.clone(),
                            content: format!("BM25 match (score: {:.2})", result.score),
                            score: result.score,
                            line_number: None,
        ...
        all_results.truncate(self.config.simple_max_results);
        
        Ok(all_results)
    }
    
    /// Index a project directory
    /// NOTE: Current BM25 implementation requires manual document addition via add_document()
    /// For full indexing capabilities including automatic directory traversal, use UnifiedSearcher
    pub async fn index_project(&mut self, _project_path: &PathBuf) -> Result<()> {
        // ARCHITECTURAL LIMITATION: The production BM25Engine requires manual document indexing
        // via add_document() method rather than automated directory indexing.
        // SimpleSearcher is designed for basic search with minimal setup.
        // For comprehensive indexing including directory traversal, use UnifiedSearcher instead.
        
        warn!("SimpleSearcher indexing not fully implemented. Use UnifiedSearcher for comprehensive indexing.");
        
        // Index with Tantivy (if available)
        #[cfg(feature = "tantivy")]
        if let Some(ref mut engine) = self.tantivy_engine {
            engine.index_directory(_project_path).await
