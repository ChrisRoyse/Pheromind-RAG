...
impl TantivySearcher {
    ...
    pub async fn new() -> Result<Self> {
        Self::new_with_config(SearchConfig::default()).await
    }
    
    /// Create a new TantivySearcher with custom configuration
    pub async fn new_with_config(search_config: SearchConfig) -> Result<Self> {
    ...
    pub async fn new_with_path<P: AsRef<Path>>(index_path: P) -> Result<Self> {
        Self::new_with_path_and_config(index_path, SearchConfig::default()).await
    }
    
    /// Create a new TantivySearcher with persistent file-based storage and custom configuration
    pub async fn new_with_path_and_config<P: AsRef<Path>>(index_path: P, search_config: SearchConfig) -> Result<Self> {
    ...
    pub async fn new_with_root<P: AsRef<Path>>(project_root: P) -> Result<Self> {
        Self::new_with_root_and_config(project_root, SearchConfig::default()).await
    }
    
    /// Create a new TantivySearcher with project root scoping and custom configuration
    pub async fn new_with_root_and_config<P: AsRef<Path>>(project_root: P, search_config: SearchConfig) -> Result<Self> {
    ...
    pub async fn search(&self, query: &str) -> Result<Vec<ExactMatch>> {
        ...
        let top_docs = searcher.search(&parsed_query, &TopDocs::with_limit(self.search_config.tantivy_max_results))?;
    ...
    pub async fn search_fuzzy(&self, query: &str, max_distance: u8) -> Result<Vec<ExactMatch>> {
        ...
        // Search with configurable limit
        let top_docs = searcher.search(&combined_query, &TopDocs::with_limit(self.search_config.tantivy_max_results))?;
