...
impl BM25Engine {
    ...
    /// Save the BM25 engine state to disk for persistence
    pub async fn save_to_disk(&self, path: &std::path::Path) -> Result<()> {
        use tokio::fs;
        ...
        let serialized = bincode::serialize(&state)
            .map_err(|e| anyhow::anyhow!("Failed to serialize BM25 state: {}", e))?;
    ...
    /// Load the BM25 engine state from disk
    pub async fn load_from_disk(&mut self, path: &std::path::Path) -> Result<()> {
        use tokio::fs;
        use tokio::io::AsyncReadExt;
        
        if !path.exists() {
            // No saved state exists - this is normal for fresh installations
            return Ok(());
        }
        ...
        let state: BM25State = bincode::deserialize(&buffer)
            .map_err(|e| anyhow::anyhow!("Failed to deserialize BM25 state: {}", e))?;
    ...
    pub fn add_document(&mut self, doc: BM25Document) -> Result<()> {
        ...
        for (term, positions) in term_positions {
            ...
            self.inverted_index.entry(term.clone())
                .or_insert_with(Vec::new)
                .push(doc_term);
        }
    ...
    /// Calculate IDF (Inverse Document Frequency) for a term using BM25 formula with correction
    pub fn calculate_idf(&self, term: &str) -> f32 {
        ...
        if let Some(stats) = self.term_frequencies.get(&term_lower) {
            ...
            // CRITICAL FIX: BM25 IDF can be negative for very common terms in small corpora
            // For search functionality, we need positive scores to indicate relevance
            // Apply epsilon floor to prevent negative IDF while preserving relative ranking
            if idf <= 0.0 {
                // Use small positive epsilon that preserves term frequency ranking
                // but prevents negative total scores
                0.01 * (1.0 + df / n) // Scale by relative frequency for better ranking
            } else {
                idf
            }
        } else {
            // Term not found - return high IDF for rare/unknown terms
    ...
    /// Search for documents matching the query using preprocessed terms
    pub fn search_with_terms(&self, query_terms: &[String], limit: usize) -> Result<Vec<BM25Match>, anyhow::Error> {
        if query_terms.is_empty() {
            return Err(anyhow::anyhow!("Empty query provided to BM25 search. Query must contain valid search terms."));
        }
        
        // Find candidate documents containing query terms
    ...
    /// Search for documents matching the query (legacy method)
    pub fn search(&self, query: &str, limit: usize) -> Result<Vec<BM25Match>, anyhow::Error> {
        // Tokenize query (simple whitespace split - DEPRECATED, use search_with_terms instead)
        ...
        self.search_with_terms(&query_terms, limit)
    }
...
pub struct BM25State {
    ...
    pub term_frequencies: FxHashMap<String, TermStats>,
    pub document_lengths: FxHashMap<String, usize>,
    pub inverted_index: FxHashMap<String, Vec<DocumentTerm>>,
}
