...
impl Config {
    ...
    pub fn new_test_config() -> Self {
        Self {
            ...
            // Search configuration with explicit defaults
    ...
    pub fn search_config() -> EmbedResult<SearchConfig> {
        Ok(CONFIG.read().map_err(|e| EmbedError::Concurrency {
            message: format!("Failed to acquire read lock for CONFIG: {}", e),
            operation: Some("get_search_config".to_string()),
        })?.as_ref().ok_or_else(|| EmbedError::Configuration {
            message: "Configuration not initialized. Call Config::init() first.".to_string(),
            source: None,
        })?.search.clone())
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if let Err(e) = self.search.validate() {
            return Err(anyhow!("Search configuration validation failed: {}", e));
        }
