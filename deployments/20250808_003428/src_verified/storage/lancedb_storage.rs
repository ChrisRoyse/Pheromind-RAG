    ...
    pub async fn search_similar_with_options(&self, query_embedding: Vec<f32>, options: SearchOptions) -> Result<Vec<LanceEmbeddingRecord>, LanceStorageError> {
        ...
        while let Some(batch) = stream.try_next().await
            ...
            for i in 0..batch.num_rows() {
                ...
                // LanceDB returns L2 distance, we need to convert to cosine similarity properly
                let similarity_score = batch.column_by_name("_distance")
                    .and_then(|col| col.as_any().downcast_ref::<Float32Array>())
                    .map(|arr| arr.value(i))
                    .map(|distance| {
                        // For normalized embeddings, L2 distance relates to cosine similarity as:
                        // cosine_similarity = 1 - (L2_distance^2 / 2)
                        // But for safety, we clamp the result to [0, 1] range
        ...
        // Apply minimum similarity threshold to improve search quality
        let min_similarity_threshold = 0.3; // Only return results with >30% similarity
        filtered_records.retain(|record| {
            match record.similarity_score {
                Some(score) => score >= min_similarity_threshold,
                None => {
                    log::warn!("Record {} missing similarity score during filtering", record.id);
                    false // Exclude records without similarity scores
                }
            }
        });
